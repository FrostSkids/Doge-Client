...
public class Stitcher
{
    ...
    public void doStitch() {
        ...
        for (int length = (array = astitcher$holder).length, i = 0; i < length; ++i) {
            ...
            if (!this.allocateSlot(stitcher$holder)) {
                final String s = String.format("Unable to fit: %s - size: %dx%d - Maybe try a lowerresolution resourcepack?", stitcher$holder.getAtlasSprite().getIconName(), stitcher$holder.getAtlasSprite().getIconWidth(), stitcher$holder.getAtlasSprite().getIconHeight());
    ...
    private boolean expandAndAllocateSlot(final Holder p_94311_1_) {
        ...
        if (MathHelper.roundUpToPowerOfTwo((flag6 ? this.currentHeight : this.currentWidth) + j2) > (flag6 ? this.maxHeight : this.maxWidth)) {
            return false;
        }
        Slot stitcher$slot;
        if (flag6) {
