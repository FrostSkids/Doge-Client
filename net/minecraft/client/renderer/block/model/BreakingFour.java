...
public class BreakingFour extends BakedQuad
{
    ...
    private void func_178216_a(final int p_178216_1_) {
        final int i = 7 * p_178216_1_;
        final float f = Float.intBitsToFloat(this.vertexData[i]);
        final float f2 = Float.intBitsToFloat(this.vertexData[i + 1]);
        final float f3 = Float.intBitsToFloat(this.vertexData[i + 2]);
        float f4 = 0.0f;
        float f5 = 0.0f;
        switch (this.face) {
            case DOWN: {
        ...
        this.vertexData[i + 4] = Float.floatToRawIntBits(this.texture.getInterpolatedU(f4));
        this.vertexData[i + 4 + 1] = Float.floatToRawIntBits(this.texture.getInterpolatedV(f5));
    }
}
