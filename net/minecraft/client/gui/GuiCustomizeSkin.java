...
public class GuiCustomizeSkin extends GuiScreen
{
    ...
    public void initGui() {
        ...
        for (int length = (values = EnumPlayerModelParts.values()).length, j = 0; j < length; ++j) {
            ...
            this.buttonList.add(new ButtonPart(enumplayermodelparts.getPartId(), GuiCustomizeSkin.width / 2 - 155 + i % 2 * 160, this.height / 6 + 24 * (i >> 1), 150, 20, enumplayermodelparts, (ButtonPart)null));
            ++i;
        }
        if (i % 2 == 1) {
            ++i;
        }
        this.buttonList.add(new GuiButton(200, GuiCustomizeSkin.width / 2 - 100, this.height / 6 + 24 * (i >> 1), I18n.format("gui.done", new Object[0])));
    }
