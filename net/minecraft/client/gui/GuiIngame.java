...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(final float partialTicks) {
        ...
        final String pattern = "hh:mm:ss a z";
        final SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
        final String time = simpleDateFormat.format(new Date());
        GuiIngame.Xpos = 5;
        this.getFontRenderer().drawStringWithShadow(String.valueOf(ClientInfos.ClientColorMods) + ClientInfos.ClientName, (float)GuiIngame.Xpos, 5.0f, -1);
        if (ClientInfos.Coords) {
            GuiIngame.Xpos += 10;
            this.getFontRenderer().drawStringWithShadow("§f[" + ClientInfos.ClientColorMods + "X§f] §f" + Math.round(Minecraft.getMinecraft().thePlayer.posX * 1000.0) / 1000L, 5.0f, (float)GuiIngame.Xpos, -1);
            GuiIngame.Xpos += 10;
            this.getFontRenderer().drawStringWithShadow("§f[" + ClientInfos.ClientColorMods + "Y§f] §f" + Math.round(Minecraft.getMinecraft().thePlayer.posY * 1000.0) / 1000L, 5.0f, (float)GuiIngame.Xpos, -1);
            GuiIngame.Xpos += 10;
            this.getFontRenderer().drawStringWithShadow("§f[" + ClientInfos.ClientColorMods + "Z§f] §f" + Math.round(Minecraft.getMinecraft().thePlayer.posZ * 1000.0) / 1000L, 5.0f, (float)GuiIngame.Xpos, -1);
            GuiIngame.Xpos += 10;
        }
        if (ClientInfos.FPS) {
            this.getFontRenderer().drawStringWithShadow("§f[" + ClientInfos.ClientColorMods + "FPS" + "§f] " + Minecraft.getDebugFPS(), 5.0f, (float)GuiIngame.Xpos, ClientInfos.ClientColorInt);
            GuiIngame.Xpos += 10;
        }
        if (ClientInfos.CPS) {
            this.getFontRenderer().drawStringWithShadow("§f[" + ClientInfos.ClientColorMods + "CPS" + "§f] " + this.getCPS(), 5.0f, (float)GuiIngame.Xpos, ClientInfos.ClientColorInt);
            GuiIngame.Xpos += 10;
        }
        if (ClientInfos.Biom) {
            this.getFontRenderer().drawStringWithShadow("§f[" + ClientInfos.ClientColorMods + "Biome" + "§f] " + chunk.getBiome(blockpos, this.mc.theWorld.getWorldChunkManager()).biomeName, 5.0f, (float)GuiIngame.Xpos, ClientInfos.ClientColorInt);
            GuiIngame.Xpos += 10;
        }
        if (ClientInfos.Time) {
            this.getFontRenderer().drawStringWithShadow("§f[" + ClientInfos.ClientColorMods + time + "§f] ", 5.0f, (float)GuiIngame.Xpos, ClientInfos.ClientColorInt);
            GuiIngame.Xpos += 10;
        }
        if (ClientInfos.ToggleSprint) {
            this.getFontRenderer().drawStringWithShadow(new StringBuilder().append(this.mc.thePlayer.movementInput.getDisplayText()).toString(), 5.0f, (float)GuiIngame.Xpos, ClientInfos.ClientColorInt);
            GuiIngame.Xpos += 10;
        }
        final ScaledResolution sr = new ScaledResolution(this.mc);
        pos.X = sr.getScaledWidth() - 80;
        final Keystrokes ks = new Keystrokes();
        ks.draw();
        for (int o = 0; o < this.mc.thePlayer.inventory.armorInventory.length; ++o) {
            final ItemStack itemStack = this.mc.thePlayer.inventory.armorInventory[o];
            this.renderItemStack(sr.getScaledHeight() - 70, 10, o, itemStack);
        }
        ...
        if (!collection.isEmpty()) {
            ...
            for (final PotionEffect potioneffect : this.mc.thePlayer.getActivePotionEffects()) {
                ...
                if (potion.hasStatusIcon()) {
                    ...
                    this.drawTexturedModalRect(X + offsetX - 20, Y + k3 - offsetY, 0 + i2 % 8 * 18, 198 + i2 / 8 * 18, 18, 18);
                }
                ...
                this.getFontRenderer().drawStringWithShadow(s1, (float)(X + offsetX), (float)(Y + k3 - offsetY), 16777215);
                final String s2 = Potion.getDurationString(potioneffect);
                this.getFontRenderer().drawStringWithShadow(s2, (float)(X + offsetX), (float)(Y + k3 + 10 - offsetY), 8355711);
                k3 += l2;
            }
        }
        if (this.field_175195_w > 0) {
            ...
            if (this.field_175195_w > this.field_175193_B + this.field_175192_A) {
                ...
                i3 = (int)(f5 * 255.0f / this.field_175199_z);
            }
            if (this.field_175195_w <= this.field_175193_B) {
                i3 = (int)(f4 * 255.0f / this.field_175193_B);
            }
            i3 = MathHelper.clamp_int(i3, 0, 255);
            if (i3 > 8) {
                GlStateManager.pushMatrix();
        ...
        if (scoreplayerteam != null) {
            final int i4 = scoreplayerteam.getChatFormat().getColorIndex();
            if (i4 >= 0) {
                scoreobjective = scoreboard.getObjectiveInDisplaySlot(3 + i4);
            }
        }
    ...
    private void renderItemStack(final int x1, final int x, final int y, final ItemStack itemStack) {
        if (itemStack == null) {
            return;
        }
        GL11.glPushMatrix();
        final int yAdd = -16 * y + 48;
        RenderHelper.enableGUIStandardItemLighting();
        this.mc.getRenderItem().renderItemAndEffectIntoGUI(itemStack, x, x1 + yAdd);
        GL11.glPopMatrix();
    }
