...
public class ScreenChatOptions extends GuiScreen
{
    ...
    public void initGui() {
        ...
        GameSettings.Options[] field_146399_a;
        for (int length = (field_146399_a = ScreenChatOptions.field_146399_a).length, j = 0; j < length; ++j) {
            final GameSettings.Options gamesettings$options = field_146399_a[j];
            if (gamesettings$options.getEnumFloat()) {
                this.buttonList.add(new GuiOptionSlider(gamesettings$options.returnEnumOrdinal(), ScreenChatOptions.width / 2 - 155 + i % 2 * 160, this.height / 6 + 24 * (i >> 1), gamesettings$options));
            }
            else {
                this.buttonList.add(new GuiOptionButton(gamesettings$options.returnEnumOrdinal(), ScreenChatOptions.width / 2 - 155 + i % 2 * 160, this.height / 6 + 24 * (i >> 1), gamesettings$options, this.game_settings.getKeyBinding(gamesettings$options)));
            }
            ++i;
        }
        this.buttonList.add(new GuiButton(200, ScreenChatOptions.width / 2 - 100, this.height / 6 + 120, I18n.format("gui.done", new Object[0])));
    }
